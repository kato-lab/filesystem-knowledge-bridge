...
以下はownerを `tkato` とする例です。

```bash
mkdir -p /mnt/knowledge-incoming/users/tkato/sample-project
cp -a examples/sample_project/. \
  /mnt/knowledge-incoming/users/tkato/sample-project/
chmod -R a+rX /mnt/knowledge-incoming/users/tkato/sample-project
```
...
```bash
uv run python scripts/mcp_test.py incoming --owner tkato
```
...
```text
/incoming/users/tkato/sample-project/
    ↓ コピー（incomingは残す）
/knowledge/users/tkato/sample-project/
    ↓ kb_index.index_directory()
private_tkato_sample-project Collectionを作成
```
...
```json
{
  ...
  "stored_path": "/knowledge/users/tkato/sample-project",
  "collection": "private_tkato_sample-project",
  "nodes": 6
}
```
...
```bash
test -d /mnt/knowledge-incoming/users/tkato/sample-project
test -d /mnt/knowledge/users/tkato/sample-project
```
...
```bash
curl -fsS \
  http://localhost:6333/collections/private_tkato_sample-project
```
...
```bash
uv run python scripts/mcp_test.py projects --owner tkato
```

少なくとも次が含まれることを確認します。

```text
private_tkato_sample-project
```
...
- 結果が最大5件
- `collection` が `private_tkato_sample-project`
...
```text
labknowledge://users/tkato/sample-project/src/main.py
```
